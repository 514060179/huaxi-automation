...
class LoginRunner:
    ...
    def _logout(self, window: MiniProgramWindow) -> None:
        for _ in range(2):
            ...
            if not self._dismiss_unbound_modal(window):
                break
            time.sleep(1.5)
        self._wait_for_any_text(
            window,
    ...
    def _wait_for_text(
        ...
        *,
        ...
        while time.monotonic() < deadline:
            if any(text in item or item in text for item in self._texts(window)):
                return
            time.sleep(1)
    ...
    def _wait_for_any_text(
        ...
        *,
        ...
        while time.monotonic() < deadline:
            ...
            if any(
                expected in text or text in expected
    ...
    def _wait_for_logged_in(
        ...
        *,
        ...
        deadline = time.monotonic() + timeout_seconds
        while time.monotonic() < deadline:
            if self._looks_logged_in(self._texts(window)):
                return
            time.sleep(1)
    ...
    def _looks_logged_in(texts: list[str]) -> bool:
        ...
        return any(marker in text for text in texts for marker in logged_markers)
    ...
    def _dismiss_unbound_modal(self, window: MiniProgramWindow) -> bool:
        ...
        while time.monotonic() < deadline:
            ...
            time.sleep(2.0)
            verify_path = self._snapshot(window, "unbound-modal-verify")
            verify_texts = [obs.text for obs in recognize_text(verify_path)]
            if "账号未绑定" not in verify_texts:
                return True
            time.sleep(1)
        return False
    ...
    def _click_text(self, window: MiniProgramWindow, text: str) -> None:
        ...
        for obs in recognize_text(path):
            if text in obs.text or obs.text in text:
                click_screen_point(
                    self._window.bounds.x + obs.center[0] / 2,
                    self._window.bounds.y + obs.center[1] / 2,
                )
    ...
    def _click_any_text(self, window: MiniProgramWindow, texts: list[str]) -> None:
        ...
        for obs in recognize_text(path):
            if any(expected in obs.text or obs.text in expected for expected in texts):
                click_screen_point(
                    self._window.bounds.x + obs.center[0] / 2,
                    self._window.bounds.y + obs.center[1] / 2,
                )
